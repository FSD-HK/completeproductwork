...
@WebServlet(name = "QueryAllTypeServlet", value = "/QueryAllTypeServlet")
public class QueryAllTypeServlet extends HttpServlet {
    @Override
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        response.setContentType("text/html;charset=utf-8");//编码方式，在请求转发forward前不做任何输出和输出语句命名
        request.setCharacterEncoding("utf-8");
        IProductService service = new ProductService();
        ArrayList<TypeDto> product = new ArrayList<TypeDto>();
